...
export default function ScrollTop() {
    ...
    useEffect(() => {
        ...
        const handleScroll = ({ scroll }) => {
            ...
            const progress = Math.round(lenis?.progress * 100) || 0;

            setPercent(progress);

            // Quay SVG theo phần trăm scroll
            if (svgRef.current) {
                svgRef.current.style.transform = `rotate(${progress * 10}deg)`;
            }

            // Hiệu ứng cho container khi scroll > 100px
    ...
    return (
        <div ref={containerRef} onClick={scrollTop} className="cursor-pointer hidden fixed right-[1.5rem] bottom-[1.5rem] z-[100] transition-opacity ease duration-[1000ms]">
            <button ref={buttonRef} key={percent} className="cursor-pointer z-[101] w-[85px] h-[85px] rounded-full text-gray-300 font-bold text-[1.1rem] transition-all ease-in-out duration-[2000ms]">
                <p className='relative top-[-2px]'>{percent}%</p>
            </button>
